...
class Util(object):
    ...
    def split_data(self, total_data):
        ...
        # Assuming total_data is a tuple where the first element is the DataFrame
        total_data_df = total_data[0] if isinstance(total_data, tuple) and len(total_data) > 0 else None
    
        if total_data_df is None:
            raise ValueError("Invalid total_data format. Please provide a tuple containing a DataFrame.")
    
        # Shuffle the DataFrame indices
        shuffled_indices = total_data_df.index.to_list()
        random.shuffle(shuffled_indices)
    
        # Use the shuffled indices to rearrange the DataFrame
        total_data_shuffled = total_data_df.loc[shuffled_indices].reset_index(drop=True)
    
        n = len(total_data_shuffled)
        print("Total size of the data is: {0}".format(n))
    
        size_train = int(n * 0.75)
        X_train = total_data_shuffled.iloc[:size_train]
        X_valid = total_data_shuffled.iloc[size_train:]
